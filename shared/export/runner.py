...
class ExportRunner:
    ...
    def start(
        ...
        """
        if not self.export_settings or not self.context:
            raise RuntimeError("ExportRunner missing config or context")

        self.generator = self._iterate_variants(collection, export_root)
    ...
    def _iterate_variants(
        ...
        try:
            ...
            if original_shape_keys and mannequin and isinstance(mannequin.data, Mesh):
    ...
    def _process_single_variant(
        ...
        dup = duplicate_collection(collection)
    ...
    def _run_variant_steps(
        ...
        run_preprocessing(self.context, list(dup.objects))

        self._check_cancel()

        # Export
        name = build_export_name(self.export_settings,
                                 collection, shapekeys_names)
